...
NP=84       # number of participants (always 1 more than the number of participants)
...
for r1 in range(0, NR):
    for n1 in range(0,NP-1):
        RT_Data = pd.read_csv('RT_Focal.csv')
        participantHand_Data = pd.read_csv('participantHand_Focal.csv')
        MyCondition_Data = pd.read_csv('Condition_Focal.csv')
        # here, I take care of the conditions
        # condition = (Radius_Data==3.3) & (NumberOfAgents_Data==4)
        cond1=(MyCondition_Data==1)
        cond2=(MyCondition_Data==2)
        cond3=(MyCondition_Data==3)
        cond4=(MyCondition_Data==4)
        Condition_Data = pd.DataFrame(np.where(cond1, '4', 
                                               np.where(cond2, '1', 
                                                        np.where(cond3, '2', '3'))),
                          index=MyCondition_Data.index, columns=MyCondition_Data.columns)
...
np.save('Focal_Low_NA7.npy', fitted_params)
